...
type: Orientation
title: Package (0.7)
description: "microscrap/mpsse 0.7.0 — bindings-only MPSSE SPI/I2C/GPIO helpers; helpers → MPSSE → FTDI; no ServiceProvider."
resource: .
tags: [orientation, mpsse, microscrap, bindings, 0.7]
...
Composer package `microscrap/mpsse` at **0.7.0** — pure-PHP MPSSE SPI / I²C / GPIO helpers modeled on [libmpsse](https://github.com/devttys0/libmpsse), built on [`microscrap/ftdi`](https://github.com/microscrap/ftdi) and **ext-ftdi**.[^composer][^readme]
...
| Require | `ext-ftdi` `^0.7.0`, `microscrap/ftdi` `^0.7.0`[^composer] |
...
- Not higher UART/USB/MPSSE adapters — those belong in `scrapyard-io/gpio-framework` (suggested peer).[^composer]
