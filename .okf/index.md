...
Package knowledge for `microscrap/mpsse` (bindings-only MPSSE SPI/I2C/GPIO helpers over `microscrap/ftdi` / **ext-ftdi**, v0.7.0).
...
**Scope:** Document the bindings-only MPSSE helpers package. Do **not** invent ServiceProviders, chip drivers, gpio-framework internals, or Fabricate remaps here — those belong in peers (`scrapyard-io/gpio-framework`, tubes).
**Dist note:** `.okf/` and root `AGENTS.md` are `export-ignore` in `.gitattributes` so Composer dist packages do not ship this bundle.

# Orientation

* [Package (0.7)](orientation/package.md) - Composer identity, namespace, helpers over MPSSE / FTDI.
